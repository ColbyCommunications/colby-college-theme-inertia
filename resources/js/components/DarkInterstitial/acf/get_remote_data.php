...
function colby_block_dark_interstitial_get_remote_data(array $data, int $index, array $block = []): array
{
    $dataset_key = 'none';

    if (!empty($data['dataset_source']) && $data['dataset_source'] !== 'none') {
        $dataset_key = $data['dataset_source'];
    } elseif (!empty($data['field_dark_interstitial_dataset_source']) && $data['field_dark_interstitial_dataset_source'] !== 'none') {
        $dataset_key = $data['field_dark_interstitial_dataset_source'];
    } else {
        foreach ($data as $val) {
