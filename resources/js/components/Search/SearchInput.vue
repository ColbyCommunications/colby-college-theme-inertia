    ...
    <div class="flex justify-between flex-col md:flex-row">
        ...
        >
            <label class="relative flex text-[0] w-full max-w-sm">
                <svg
                    class="absolute top-3 left-3 fill-indigo-800 w-2.5"
                    viewBox="0 0 9.6 9.6"
                    aria-hidden="true"
                >
                    <path
...
<script setup>
    ...
    /* Props (kept 1:1 with the original for drop-in compatibility) */
    const props = defineProps({
        placeholder: { type: String, default: 'Search here…' },
        autofocus: { type: Boolean, default: false },
        showLoadingIndicator: { type: Boolean, default: false },
        shouldShowLoadingIndicator: { type: Boolean, default: false },
    ...
    /* SUIT helper (replacement for createSuitMixin({ name: 'SearchBox' })) */
    function suit(element) {
        // Vue InstantSearch style: `ais-SearchBox`, `ais-SearchBox-form`, `ais-SearchBox-input`, etc.
        return element ? `ais-SearchBox-${element}` : 'ais-SearchBox';
    }

    /* Handlers */
    function onFormSubmit() {
        // blur the input to close mobile keyboards etc.
        inputRef.value?.blur();
    }
