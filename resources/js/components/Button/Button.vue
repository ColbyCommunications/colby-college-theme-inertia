...
<template>
  <a
    ...
    class="btn group inline-flex flex-row items-center rounded border-[0.75px] border-solid font-body leading-130 font-normal !no-underline outline-offset-[-1px] transition-all duration-200 ease-in-out focus:outline focus:outline-2 focus:outline-gold min-w-[44px] min-h-[44px]"
    :class="[
      space,
      textSize,
      paddingY,
...
const textColor = computed(() => (props.transparentBg ? "text-white" : isDark.value ? "text-white" : "text-indigo"));
...
const hoverBorderColor = computed(() =>
  props.transparentBg ? "" : isDark.value ? "hover:border-indigo" : "hover:border-snow",
);
const hoverTextColor = computed(() =>
  props.transparentBg ? "hover:text-indigo" : isDark.value ? "hover:text-indigo" : "hover:text-indigo",
);
const hoverBackgroundColor = computed(() =>
  props.transparentBg ? "hover:bg-white" : isDark.value ? "hover:bg-white" : "hover:bg-cloud",
);
