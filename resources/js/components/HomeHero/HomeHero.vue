<template>
  ...
  >
    ...
    >
      ...
      >
        <div
          class="font-extended text-right text-12 font-bold tracking-8 text-gold uppercase md:text-left"
        >
          {{ caption }}
        </div>
      </div>
...
const selectedVideo = computed(() => {
  ...
  if (isMobile.value) {
    return props.video_mobile || props.video_tablet || props.video || "";
  }
...
const selectedHlsVideo = computed(() => {
  if (isMobile.value) {
    return (
      props.hls_video_mobile || props.hls_video_tablet || props.hls_video || ""
    );
  }
...
// Only render the fallback Picture component if NO video sources exist at all.
const showPosterImage = computed(
  () => !selectedVideo.value && !selectedHlsVideo.value,
);
